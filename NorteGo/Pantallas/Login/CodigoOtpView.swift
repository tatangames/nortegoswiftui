...
struct CodigoOtpView: View {
    ...
    @State private var boolPantallaPrincipal = false
    ...
    // constructor
    init(initialTime: Int, phoneNumber: String) {
        _timerViewModel = StateObject(wrappedValue: TimerViewModel(initialTime: initialTime))
        self.phoneNumber = phoneNumber
    }
    
    // una Vez reintento verificar numero
    @State private var boolApiVerificarSMS = true
    // una Vez reintento de enviar codigo
    @State private var boolApiReenvioSMS = true
    ...
    var body: some View {
        ...
        NavigationView {
            ZStack {
                ScrollView {
                    VStack(spacing: 15){
                        Text("Verifica \(phoneNumber)")
                            .foregroundColor(Color("c_azulv2"))
                        ...
                        Text(timerViewModel.tiempoSegundo == 0 ? "Reenviar Código" : "Reintentar en \(timerViewModel.tiempoSegundo)")
                            ...
                            .onAppear {
                                timerViewModel.startTimer() // Iniciar el temporizador al aparecer la vista
                            }
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(action: {
                        // Acción para el botón de la izquierda
                        dismiss()
                    }) {
            ...
            .fullScreenCover(isPresented: $boolPantallaPrincipal) {
