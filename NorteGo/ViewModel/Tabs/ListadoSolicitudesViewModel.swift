//
//  ListadoSolicitudesViewModel.swift
//  NorteGo
//
...
class ListadoSolicitudesViewModel: ObservableObject {
    @Published var jsonResponse: JSON?
    @Published var loadingSpinner: Bool = false
    @Published var isRequestInProgress: Bool = false
    @Published var error: Error?
    @Published var misSolicitudesArray: [ModeloListadoSolicitudes] = []
    
    private let disposeBag = DisposeBag()
     
    func misSolicitudesRX(idToken: String, idcliente: Int, completion: @escaping (Result<JSON, Error>) -> Void) {
